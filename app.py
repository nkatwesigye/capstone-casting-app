...
def create_app(test_config=None):
  ...
  def movies_get(payload):
      ...
      ,Movies.title,Movies.release_date,Actors.name).join(Actors \
      ,(Movies.actor_id == Actors.id)).all()
  ...
  def movies(payload):
      if request.get_json().get('movie_name'):
          ...
          movie =  Movies(title=movie_name,release_date=release_date,actor_id=id_actor)
          movie.insert()
          movie_id = movie.id
  ...
  def update_movie_by_id(payload,movie_id):
      ...
      if movie_by_id is None:
         abort(409)
      try:
       if request.get_json().get('new_movie_name') and  request.get_json().get('new_release_date'):
          body= request.get_json()
  ...
  def update_actor_by_id(payload,actor_id):
      ...
      if actor_by_id is None:
         abort(404)
      try:
       if request.get_json().get('new_actor_name') and  request.get_json().get('new_actor_age'):
         body= request.get_json()
